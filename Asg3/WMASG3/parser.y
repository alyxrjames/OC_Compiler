...
structdef: TOK_STRUCT TOK_IDENT '{' '}'
	{$$ = adopt1($1,adopt1_sym($2,NULL,TOK_TYPEID)); free_ast2($3,$4);}
	declarations '}' {$$ = $1; free_ast($2);}
	;

declarations: TOK_STRUCT TOK_IDENT '{' identdecl ';'
	{$$ = adopt2($1,adopt1_sym($2,NULL,TOK_TYPEID),$4); free_ast2($3,$5);}
	| declarations identdecl ';' {$$ = adopt1($1,$2); free_ast($3);}
	;
...
param	: param ',' identdecl {$$ = adopt1($1,$3);free_ast($2);}
	| '(' identdecl {$$ = adopt1_sym($1,$2, TOK_PARAMLIST);}
	;
...
allocator: TOK_NEW TOK_IDENT '(' ')' {$$ = adopt1($1,adopt1_sym($2, NULL, TOK_TYPEID));
				     free_ast2($3,$4);} 
	 | TOK_NEW TOK_STRING '(' expr ')' {$$ = adopt1_sym($1,$4,TOK_NEWSTRING);
					   free_ast($2);free_ast($3);free_ast($5);}
	 | TOK_NEW basetype '[' expr ']' {$$ = adopt1(adopt1_sym($1,$2, TOK_NEWARRAY),$4);
					 free_ast2($3,$5);}
	 ;
...
variable: TOK_IDENT {$$ = $1;}
	...
	| expr '.' TOK_IDENT {$$ = adopt2($2,$1,adopt1_sym($3,NULL,TOK_FIELD));}
	;
