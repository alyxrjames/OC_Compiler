...
astree* new_prototype(astree * node){

	astree * tree = new astree(TOK_PROTOTYPE,
	node->filenr,node->linenr, node ->offset, "");
	return tree;
}
astree* new_func(astree * node){

	astree *  tree = new astree(TOK_FUNCTION,
	node->filenr,node->linenr,node->offset,"");
	return tree;
}
